...
<body>
	<form class="container signup body" action="<%=request.getContextPath()%>/mypage" method="post">
		<h1 class="title text-center">회원정보 수정 member detail modification</h1>
		...
		<div class="form-group">
			<input type="text" class="form-control" placeholder="name" name="me_name" value="${user.me_name}">
		</div>
		<div class="form-group">
			<input type="text" class="form-control" placeholder="Date of Birth" name="me_birth" id="birth" value="${user.me_birth}">
		</div>
		<div class="form-group">
			<div class="form-check-inline">
				<label class="form-check-label">
					<input type="radio" class="form-check-input" name="me_gender" value="male" <c:if test="${user.me_gender == 'male' }">checked</c:if>>male
				</label>
			</div>
			<div class="form-check-inline">
				<label class="form-check-label">
					<input type="radio" class="form-check-input" name="me_gender" value="female" <c:if test="${user.me_gender == 'female' }">checked</c:if>>female
				</label>
			</div>
		...
		<button class="btn btn-outline-success col-12">회원정보 수정 member detail modification</button>
	</form>
<script>
	$('form').submit(function(){
		...
		if(me_pw != me_pw2){
			alert('비밀번호가 일치하지 않습니다. does not match.');
			return false;
		}
		...
		if(me_pw == '' || me_address == ' '){
			return confirm('입력되지 않은 비밀번호 또는 주소는 이전 정보로 저장됩니다. 진행하시겠습니까? Do you want to proceed?')
		}
	});
